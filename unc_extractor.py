...
class ViTExtractor:
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        elif 'unc' in model_type:
            print("Loading uncertainty model")
            model = torch.hub.load('facebookresearch/dino:main', 'dino_vits8')
            path = '/home/singhk/url/weights/vit_small_checkpoint.pth.tar'
            load_pretrained_weights(model, path, 'student', "dino_vits8", 8)
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        ...
        _, unc = self.model(batch)
        self._unregister_hooks()
        self.load_size = (H, W)
        self.num_patches = (1 + (H - self.p) // self.stride[0], 1 + (W - self.p) // self.stride[1])
        return self._feats, unc
    ...
    def extract_descriptors(self, batch: torch.Tensor, layer: int = 11, facet: str = 'key',
        ...
        _, unc = self._extract_features(batch, [layer], facet)
        x = self._feats[0]
        if facet == 'token':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', default='dino_vits8', type=str,
                        ...
                        vit_small_patch16_224 | vit_base_patch8_224 | vit_base_patch16_224 | kelpie | suim | unc_vits8]""")
